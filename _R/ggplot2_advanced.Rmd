...
data <- 
  tibble(
    group = factor(c(rep("Group A", 100), rep("Group B", 250), rep("Group C", 25))),
    type = factor(c(rep(c("Type X","Type Y"), 187), "Type Y")),
...
ggplot(economics_long, aes(value)) +
  ...
  geom_histogram(binwidth=function(x) 2 * IQR(x) / (length(x)^(1/3)), color="white")  +
  scale_y_continuous(expand=expansion(mult=c(0,.05))) +
  labs(title="Histograms with facetted binwidth",
...
df.plt <- data |> 
  mutate(
    ...
    facet.lbl = fct_reorder(paste0(group, " (n=", n(), ")"), as.numeric(group)), 
    .by=group
  )
...
p1 <- 
  ...
  geom_histogram(aes(y=after_stat(density), fill=facet.lbl), color="white", show.legend=F,
                 # binwidth=function(x) 2 * IQR(x) / (length(x)^(1/3)),
                 bins=n_bins) +
  
  geom_vline(aes(xintercept=media), color="black") +
  # geom_vline(aes(xintercept=q1), color="black") +
  # geom_vline(aes(xintercept=q3), color="black") +
  # geom_vline(aes(xintercept=mediana), color="blue") +
    
  # Overlay histogram of all observations
  geom_histogram(data=data, aes(y=after_stat(density), x=value), alpha=.4, 
                 # binwidth=function(x) 2 * IQR(x) / (length(x)^(1/3)),
                 bins=n_bins) +
...
p1 + 
  labs(title="Facetted Histograms with overlaid Gaussian and Mean",
       subtitle = "Shaded gray histograms show the distribution of all data over each group",
       x=NULL, y=NULL)
```
