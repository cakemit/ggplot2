...
theme_update(
  ...
  plot.subtitle = element_text(size= 9, colour="gray50", face='italic'),
  plot.caption  = element_text(size= 7, colour="gray50", hjust=0),
...
```{r}
data <- read.table(text = "sample Types Value
sample1 A   -36
sample2 B   31
sample1 C   15
sample2 D   -12
...
data |> 
  mutate(Types = fct_reorder(Types, Value)) |>
  ggplot(aes(x=Value, y=Types)) +
  geom_col() +
  # FOR DATA LABELS:
  geom_text(
    aes(
      label = Value,
      # x/y mapping determines label position
      # the sign() of a real number is +1/0/−1 if the number is positive/0/negative
      # The coefficient (2) needs to be adjusted to your data
      x = Value + 2 * sign(Value) 
    ), 
    size =3) +
  labs(title="Labeled Horizontal bar chart with positive and negative values")
```

Note that `sign()` returns a vector with the signs of the corresponding elements of x (the sign of a real number is 1, 0, or −1 if the number is positive, zero, or negative, respectively).
...
I found that by surrounding the ggplot call in `{}` and passing `.` as the data argument, one can continue to refer to `.` throughout the call (see 1 and 2). 
...
data.frame(
  ...
  geom_point() +
  annotation_custom(
    grid::textGrob(label = "some label", 
                   x=.95, y=.1, vjust=1, hjust=1,
                   gp = grid::gpar(col="red", alpha=.7, fontsize=10, fontface="bold.italic"))
  ) +
  labs(title = "Fixed label position")
```

You can re-run this and observe the plot label stay fixed even as the x/y axis ranges change significantly. 
...
## Reference df inside ggplot call

Use the dot `.` to reference the df inside the ggplot call (even if it is modified).
...
mpg |>  
  ...
  geom_point() +
  geom_text(data = . %>% filter(hwy > 28), vjust=-1, color="blue") +
  labs(title = "Reference df inside ggplot call (1)")
```
\
...
mpg %>%  
  filter(manufacturer == 'audi') %>% {
    ggplot(., aes(x = hwy, y = displ, label = model)) +    
      ...
      geom_text(data = . %>% filter(hwy > 28), vjust=-1, color="blue") +
      labs(
        title = "Reference df inside ggplot call (2)",
        subtitle = glue::glue("MANUFACTURER: {unique(.$manufacturer)}"),
      )
  }
```
...
***Warning**: For `%+%` to work, make sure the column names of the new data matches the column names of the dataset being replaced.*
...
```{r fig.width=5, fig.asp=.6}
plot %+% (
  as_tibble(USArrests, rownames="States")[,c(1,3)]  |> 
    head(10) |> 
    rename(Murder=Assault) # renames the variable just for the plotting
) + 
  labs(
    y="US States",
...
**Do you need to suppress warnings or do you need to put `na.rm=TRUE` in the `geom_*()` call?**
