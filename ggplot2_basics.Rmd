...
mpg |>  ggplot(aes(displ, hwy)) +
  geom_smooth(method = "lm", se = FALSE,            # linear fit to the data point
              color = "blue", size = 1.5) +         # tuning line color and thickness
  geom_point(alpha = 0.5, size = 2) +               # tuning the transparency and size of data points
  # scale_x_continuous(breaks=seq(1960,2020,5)) +   # breaking x-axis ticks in intervals of 10
  labs(x = "Displ", y = "Highway consumption",      # adding titles to the axis
       title = "Title goes here",                   # adding title to the plot
...
p4 <- mpg |> ggplot(aes(displ, hwy)) + 
  geom_smooth(method = "lm", 
              formula = y~x+I(x^2)+I(x^3),        # user defined function
              color = "blue", size = 1.5) +
  geom_point() +
  labs(x = "Highway consumption", y = "Avg. Temperature (°C)")
...
```{r fig.width = 5, fig.height = 3}
data |> 
  ggplot(aes(x=quarter, y=profit, group = product)) +
...
*Too many lines! Need gradient colors*

I have data with about 100 ordered categories.
I would like to plot each category as a line separately, with the line colors ranging from a low value (say, blue) to a high value (say, red).
...
And you can change the label names on the legend:
...
data |> group_by(quarter) |> 
  summarise(profit = sum(profit), .groups = "drop") |> 
  ggplot(aes(x=quarter, y=profit)) +
  # geom_col() == geom_bar(stat = "identity")
  # geom_col() + # default = stacked bars
  geom_bar(stat = 'identity', fill = "lightblue") + 
  geom_text(aes(label=profit), vjust = -0.5, size = 4) +
  scale_y_continuous(expand = c(0,2))
```


You can adjust the width of the bar. The default width between bars is 0.9. The closer the width is to 1, the closer together the bars will be. The close the width is to 0, the more spread out the bars will be.
...
Adjust Spacing Between Bars in Clustered Bar Chart: The width value controls the spacing between clusters while the position_dodge() value controls the spacing between bars within the same cluster.
...
Or you can set space between bars: As of ggplot 3.0.0, you can use position_dodge2.
It takes a padding argument that adds space in between the bars.
...
To change the coloring, you only need to change the fill value in the data layer.
Here's an example:

```{r fig.height=4, fig.width=10}
p1 <- data |> 
  ggplot(aes(x=quarter, y=profit, fill = product)) +
  ...
  geom_text(aes(label=scales::number(after_stat(y)),
                group = quarter),
...
p2 <- data |> 
  mutate(pct = profit/sum(profit), .by = quarter) |> 
  ggplot(aes(x=quarter, y=pct, fill = product)) +
  geom_col(color = "white") +
  geom_text(aes(label=scales::percent(pct, accuracy = 1), group=product), 
            position = position_fill(.5)) +
  scale_fill_brewer(palette = "Dark2") +
  scale_y_continuous(labels = NULL, expand = expansion(mult = c(0,0))) +
  labs(title = "Stacked to 100%")
...
Here's a general solution, that adds a "position" column to the dataframe, to use with geom_text() and place the labels exactly where they belong:

```{r fig.width = 5, fig.height = 3}
mpg |> group_by(class, year = as.factor(year)) |> 
  tally() |> 
  ungroup() |>
  arrange(desc(class)) |> 
  group_by(year) |> 
  mutate(pos = cumsum(n) - n/2) |> # create "position" column as 1/2 the y-axis variable
  ggplot() +
...
p1 <- data |> 
  ...
  geom_text(aes(label = profit), hjust = -0.5, size = 4) + 
  scale_y_continuous(expand = c(0,2.5)) +
  coord_flip() 

p2 <- data |> 
  ggplot(aes(x=quarter, y=profit, 
             fill = product)) + # defines global fill to group everywhere by product
  geom_col(position = position_dodge()) + 
  geom_text(aes(label = profit), 
            position = position_dodge(0.9), hjust = -0.5, size = 4) +
  scale_fill_npg() +
...
```{r fig.width = 5, fig.height = 3}
ggplot(data, aes(x = quarter, y = profit, fill=product, group=product)) +
  geom_area() 
```
...
Year Courses Sold Percentage Of Students Enrolled 2014 35 30% 2015 30 25% 2016 40 30% 2017 25 50% 2018 30 40% 2019 35 20% 2020 65 60%
...
In the above plot, we can observe that the bar plot is in proper shape as expected, but the line plot is merely visible.
It happens due to the scaling factor since the line plot is for the percentage of students which is in decimal and the current vertical axis having very large values.
So, we need a secondary axis in order to fit the line properly in the same chart area.

As scaling comes into the picture, we have to use the R function scale_y_continuous( ) which comes in the ggplot2 package.
Also, another function sec_axis( ) is used to add a secondary axis and assign the specifications to it.
...
The scaling factor is the trickiest part to handle while dealing with a secondary axis.
Since the secondary axis needs to be in percentage we have to use the scale factor of 0.01 and write the formula of conversion in the trans argument of sec_axis( ).
And you are scaling with 0.01 in the formula, you also have to multiply the same axis with 100 in the geom_line( ) in order to make balance in scaling.
...
The secondary axis added will be in the form of fractional value as we can see above.
But we need the secondary axis in percentage.
In order to convert to percentage, we have to use the arguments labels inside the sec_axis( ).

Some important keywords used are :

scale: It is used for scaling the data.
A scaling factor is multiplied by the original data value.
labels: It is used to assign labels.

The function used is scale_y_continuous( ) which is a default scale in "y-aesthetics" in the library ggplot2.
Since we need to add "percentage" in the labels of the Y-axis, the keyword "labels" is used.

Now use below the command to convert the y-axis labels into percentages.

scales : : percent

This will simply scale the y-axis data from decimal to percentage.
It multiplies the present value by 100.
The scaling factor is 100.

```{r fig.width = 5, fig.height = 3}
# Plotting Multiple Charts and changing secondary axis to percentage
ggplot(perf)  + 
  geom_bar(aes(x=year, y=course),stat="identity", fill="cyan",colour="#006000")+
  geom_line(aes(x=year, y=100*penroll),stat="identity",color="red",size=2)+
  labs(title= "Courses vs Students Enrolled in GeeksforGeeks",
